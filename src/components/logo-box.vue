<template>
    <div class="logo-box" :class="{light: themeCheck.isLight, dark: themeCheck.isDark}">
        <img class="logo-image" :src="svgSrc" :alt="{'Arkyfinity logo light version': themeCheck.isLight, 'Arkyfinity logo dark version': themeCheck.isDark}" />
...
export default {
    ...
    computed: {
        ...
        svgSrc() {
            return `${this.src.slice(0, -4)}.svg`
        },

        pngSrc() {
            return `${this.src.slice(0, -4)}.png`
        }
    }
}
</script>
